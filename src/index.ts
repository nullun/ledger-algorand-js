...
export default class AlgorandApp {
  ...
  async deviceInfo(): Promise<ResponseDeviceInfo> {
    return this.transport
      .send(0xe0, 0x01, 0, 0, Buffer.from([]), [0x6e00])
      .then(response => {
        const errorCodeData = response.slice(-2)
        const returnCode = errorCodeData[0] * 256 + errorCodeData[1]
  ...
  async getPubkey(
    ...
    const p1_value = requireConfirmation
      ? P1_VALUES.SHOW_ADDRESS_IN_DEVICE
      : P1_VALUES.ONLY_RETRIEVE

    return this.transport
      .send(CLA, INS.GET_PUBLIC_KEY, p1_value, P2_VALUES.DEFAULT, data, [
        0x9000,
      ])
  ...
  async signSendChunk(
    ...
    if (chunkIdx === 1) {
      p1 =
        accountId !== 0
          ? P1_VALUES.MSGPACK_FIRST_ACCOUNT_ID
          : P1_VALUES.MSGPACK_FIRST
    }
  ...
  async sign(accountId = 0, message: string | Buffer) {
    return this.signGetChunks(accountId, message).then(chunks => {
      return this.signSendChunk(1, chunks.length, accountId, chunks[0]).then(
        async result => {
